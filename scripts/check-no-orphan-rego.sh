...
grep -q '^package tdt\.pdp$' "${POLICY_DIR}/policies.rego" || {
  echo "error: policy package must be tdt.pdp" >&2; fail=1; }

[ "$fail" -eq 0 ] && echo "OK: single authoritative policy (${POLICY_DIR}/policies.rego, package tdt.pdp)"
exit "$fail"
